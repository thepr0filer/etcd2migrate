...
// dir to dir http://127.0.0.1:2379 /dir1/ http://127.0.0.1:2379 /backup/dir1backup/
// dir to dir http://127.0.0.1:2379 / http://127.0.0.1:2379 /backup/dir1backup/
        ...
        while(processingQueue.isNotEmpty()) {
            ...
            if (current.dir) {
                ...
                if (current.source != null)
                    current = Etcd2ServerOperator(Etcd2ServerConfig(current.source!!, current.key!!)).get()
...
data class Etcd2ServerConfig(val serverURL: URL, val nodeName:String) {
...
class Etcd2MigrateConfig(args: Array<String>) {
    ...
    init {
        if (args.size != 4) throw IllegalArgumentException("Required arguments: sourceServer sourceNode destinationServer destinationNode")
        ...
        sourceServer = Etcd2ServerConfig(URL(join(URL_SEPARATOR, args[0],ETCD2_PREFIX)), args[1])
        destinationServer = Etcd2ServerConfig(URL(join(URL_SEPARATOR, args[2],ETCD2_PREFIX)), args[3])
    }
}
